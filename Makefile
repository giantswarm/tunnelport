...
generate: controller-gen goimports ## Generate code containing DeepCopy, DeepCopyInto, and DeepCopyObject method implementations.
	...
	"$(GOIMPORTS)" -local github.com/giantswarm/tunnelport -w api/
...
KIND_CLUSTER ?= tunnelport-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name tunnelport-builder
	$(CONTAINER_TOOL) buildx use tunnelport-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm tunnelport-builder
	rm Dockerfile.cross
