...
| `tbot.image.registry` / `tbot.image.name` / `tbot.image.tag` / `tbot.image.digest` | `gsoci.azurecr.io/giantswarm/tbot-distroless:18@sha256:...` | Single global tbot image for **every** rendered `RemoteApp` Deployment. Sourced from gsoci (a mirror of `public.ecr.aws/gravitational/tbot-distroless`) so rendered pods satisfy the `restrict-image-registries` Kyverno policy — override `registry` to use a different mirror. **Pinned by digest** so a re-push behind the floating `:18` tag can't silently change tbot's config schema underneath the operator. RemoteApp.spec deliberately has no per-CR override. |
